...
export function useIkigai() {
    ...
    const setNote = useCallback((id, note) => {
        update((prev) => prev.map((a) => (a.id === id ? { ...a, note } : a)));
    }, []);

    const replaceAll = useCallback((next) => update(() => (Array.isArray(next) ? next : EMPTY)), []);

    const reset = useCallback(() => update(() => EMPTY), []);

    return { activities, ready, addActivity, removeActivity, renameActivity, toggleDim, setNote, replaceAll, reset };
}
